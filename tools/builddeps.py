...
def get_eqnsys(filenames):
	# Determines euqation systems out of given files
	# Specify possible types
	eqn = []
	eqn.append("linearscalaradvection")
	eqn.append("navierstokes")
	for system in eqn:
		pattern = re.compile(system, re.IGNORECASE)
		for filename in filenames:
...
def put_eqnsys(eqnsys, filenames):
  # Replaces $(EQNSYS) equation systems in given files
  # Specify possible types
  pattern = re.compile("\$\(EQNSYS\)", re.IGNORECASE)
  for i in range(len(filenames)):
    filenames[i] = pattern.sub(eqnsys, filenames[i])
  return filenames

def put_problem(problem, filenames):
  # Replaces $(PROBLEM) equation systems in given files
  # Specify possible types
  pattern = re.compile("\$\(PROBLEM\)", re.IGNORECASE)
  for i in range(len(filenames)):
    filenames[i] = pattern.sub(problem, filenames[i])
  return filenames

def assemble_deplist_output(filename, use_statements):
	# Builds GNU makefile conform deplist output
	# Restore EQNSYS-Variable
	#pattern = re.compile(eqnsys, re.IGNORECASE)
	#filename = pattern.sub("$(EQNSYS)", filename)
	#for i in range(len(use_statements)):
	#	use_statements[i] = pattern.sub("$(EQNSYS)", use_statements[i])
	deplist_output = []
...
def write_deplist_header(deplist_name):
	...
	file.write("### Deplist auto-generated by builddeps V0.99a ###\n\n")
	file.close()
...
def usage():
	...
	print ""
	print "\t -t, --target"
        print "\t\t target equation system to replace $(EQNSYS) occurances"
	print ""
	print "\t -e, --exceptions"
...
# Use getopt
try:
	opts, args = getopt.gnu_getopt(sys.argv[1:], "s:t:p:e:h", ["source=", "target=", "problem=", "exceptions=", "help"])
...
# Replace $(EQNSYS) in filenames
filenames = put_eqnsys(eqnsys, filenames)

# Replace $(PROBLEM) in filenames
filenames = put_problem(problem, filenames)
